...
class KS_solver():
    def __init__(self, initial_condition, dt, x):
    ...
    def advection(self, uhat):     
        NonLin = -0.5*np.fft.rfft(np.fft.irfft(uhat).real*np.fft.irfft(1j*self.k*uhat).real)
        #NonLin = -0.5*np.fft.rfft(np.fft.irfft(1j*self.k*uhat).real**2)
        NonLin[self.N//3:-self.N//3] = 0 
        return NonLin
